...
def create_section(
    section_data: SectionCreate, tp_id: UUID, session: Session = Depends(get_session)
):
    # Создаем секцию, привязывая её к конкретной ТП
    db_section = Section(**section_data.dict(exclude={"subscribers"}), tp_id=tp_id)
...
def read_section(section_id: UUID, session: Session = Depends(get_session)):
